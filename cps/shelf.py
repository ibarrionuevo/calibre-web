...
def search_to_shelf(shelf_id):
    ...
    if not check_shelf_edit_permissions(shelf):
        log.warning("You are not allowed to add a book to the the shelf")
        flash(_(u"You are not allowed to add a book to the that shelf"), category="error")
        return redirect(url_for('web.index'))
...
def remove_from_shelf(shelf_id, book_id):
    ...
    if check_shelf_edit_permissions(shelf):
        ...
        if not xhr:
            log.warning("You are not allowed to remove a book from shelf")
